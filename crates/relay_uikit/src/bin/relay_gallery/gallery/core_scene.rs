...
pub(super) fn render(
    ...
    scene_stack()
        ...
        .child(section(cx, "Number & Slider & Stepper", number_sample(state, host)))
        .child(section(cx, "Segmented Control", segmented_sample(state)))
        .child(section(cx, "Disclosure", disclosure_sample(state, disclosure_open)))
        .child(section(cx, "List & Tree Rows", tree_sample(state, cx)))
}

fn toast(host: Entity<GalleryScenesApp>, msg: impl Into<String>) -> impl Fn(&gpui::ClickEvent, &mut gpui::Window, &mut App) {
    let msg = msg.into();
    move |_, _, cx: &mut App| {
...
fn button_sample(host: &Entity<GalleryScenesApp>) -> impl IntoElement {
    ...
    div().flex().flex_col().gap_3()
        .child(strip()
            .child(Button::new("btn-pri", "Primary").primary().on_click(toast(h.clone(), "Primary")))
            .child(Button::new("btn-sec", "Secondary").variant(ButtonVariant::Secondary).on_click(toast(h.clone(), "Secondary")))
            .child(Button::new("btn-ghost", "Ghost").ghost().on_click(toast(h.clone(), "Ghost")))
            .child(Button::new("btn-dang", "Danger").danger().on_click(toast(h.clone(), "Danger"))))
        .child(strip()
            .child(Button::new("btn-pri-icon", "With Icon").primary().icon(IconName::Play).on_click(toast(h.clone(), "Play")))
            .child(Button::new("btn-dis", "Disabled").primary().disabled(true)))
        .child(strip()
            .child(Button::new("btn-sec-icon", "Refresh").variant(ButtonVariant::Secondary).icon(IconName::RefreshCw).on_click(toast(h.clone(), "Refresh")))
...
fn choice_sample(state: &GalleryState, host: &Entity<GalleryScenesApp>) -> impl IntoElement {
    ...
    div().flex().flex_col().gap_3()
        .child(strip()
            .child(Toggle::bound("demo-toggle", state.notifications.clone()).label("Notifications").on_click(toast(h.clone(), "Toggle toggled")))
            .child(Checkbox::bound("demo-check", state.auto_archive.clone()).label("Auto archive").on_click(toast(h.clone(), "Checkbox toggled"))))
        .child(strip()
            .child(Radio::bound("demo-radio-sys", state.radio_choice.clone(), "system", "System").on_click(toast(h.clone(), "Radio: system")))
            .child(Radio::bound("demo-radio-light", state.radio_choice.clone(), "light", "Light").on_click(toast(h.clone(), "Radio: light")))
            .child(Radio::bound("demo-radio-dark", state.radio_choice.clone(), "dark", "Dark").on_click(toast(h, "Radio: dark"))))
}
    ...
    div().flex().flex_col().gap_3()
        ...
        .child(strip()
            .child(FilterBar::new("demo-fb")
                .child(FilterChip::new("chip-all", "All").icon(IconName::LayoutGrid).on_click(toast(h.clone(), "Filter: All")))
                .child(FilterChip::new("chip-run", "running").icon(IconName::Play).selected(true).on_click(toast(h, "Filter: running")))
            ...
            .child(NumberInput::bound("demo-num", state.ui_font_size.clone())
                ...
                .on_change({
                    ...
                    move |v: i32, _: &mut gpui::Window, cx: &mut App| {
                        h.update(cx, |this, cx| this.add_feedback_toast(cx, format!("Font size: {v}px")));
                    }
                })))
        .child(strip()
            .child(Slider::bound("demo-slider", state.contrast.clone(), 0.0, 100.0)
                .on_change({
                    ...
                    move |v: f32, _: &mut gpui::Window, cx: &mut App| {
                        h.update(cx, |this, cx| this.add_feedback_toast(cx, format!("Contrast: {:.0}%", v)));
                    }
                })))
        .child(strip()
...
fn segmented_sample(state: &GalleryState) -> impl IntoElement {
    SegmentedControl::bound("demo-seg",
        vec![Segment::new("files", "Files"), Segment::new("diff", "Diff"), Segment::new("review", "Review")],
        state.seg_tab.clone())
}
