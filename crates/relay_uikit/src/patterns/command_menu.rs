...
pub enum CommandMenuItemKind {
    Terminal,
    Agent,
    Action,
}

impl CommandMenuItemKind {
    fn label(self) -> &'static str {
        match self {
            CommandMenuItemKind::Terminal => "TERM",
            CommandMenuItemKind::Agent => "AGENT",
            CommandMenuItemKind::Action => "ACTION",
        }
    }
}
...
impl CommandMenuItem {
    ...
    pub fn kind(mut self, kind: CommandMenuItemKind) -> Self {
        self.kind = kind;
        self
    }
    ...
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        for (index, item) in self.items.into_iter().enumerate() {
            ...
            let kind_color = match item.kind {
                CommandMenuItemKind::Terminal => theme.text_secondary,
...
mod tests {
    ...
    fn launcher_item_defaults_to_action() {
        let item = CommandMenuItem::new("settings", "Agent Settings", IconName::Settings);
        assert_eq!(item.kind, CommandMenuItemKind::Action);
    }

    #[test]
    fn launcher_kind_labels_are_stable() {
        assert_eq!(CommandMenuItemKind::Agent.label(), "AGENT");
    }
}
